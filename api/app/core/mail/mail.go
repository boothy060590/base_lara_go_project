...
func NewMailProvider(config *MailConfig, mailer *gomail.Dialer, queueService app_core.QueueService) *MailProvider {
	return &MailProvider{
		config:       config,
		mailer:       mailer,
...
func (m *MailProvider) SendMailAsync(to []string, subject, body string, queueName string) error {
	...
	return m.queueService.SendMessageToQueueWithAttributes(string(jobData), attributes, queueName)
}
...
var MailServiceInstance app_core.MailService

// SetMailService sets the global mail service
func SetMailService(service app_core.MailService) {
